...
namespace WorkflowProcessingModel.Model
{
    class Operation
    {
        ...
        public Operation(int index, string name, Dictionary<Material, int> materialsDemand, Dictionary<Machine, int> capableMachinesWithProductionTime, List<SetupForBatch> setupTimes,
            List<Move> timesOfMovingSemiproducts, Batch currentBatch)
        {
            Index = index;
            Name = name;
        ...
        {
            return new Operation(this.Index, this.Name + " - clone for Job with index " + jobIndex, this.MaterialsDemand, this.CapableMachinesWithProcessingTime, this.SetupTimes, this.TimesOfMovingSemiproducts, this.CurrentBatch);
        }
    }
}